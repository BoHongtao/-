    ...
    public function actionDel()
    {
        if (Yii::$app->request->isAjax) {
            ...
            if ($name) {
                ...
                if (!empty($user)) {
                    return ['code'=>0,'desc'=>'当前角色绑定管理员，请先解除绑定'];
                }
                ...
                try {
                    ...
                    $remove_child = true;
                    if (!empty($data)) {
                        $remove_child = $auth->removeChildren($role);
                    }
                    $remove_role = $auth->remove($role);
                    if (!$remove_child || !$remove_role) {
